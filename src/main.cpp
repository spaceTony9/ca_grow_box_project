...
const unsigned long MQTT_UPDATE_INTERVAL = 10000; // 10 seconds for testing
...
void setup() {
    ...
    oled_clear();
    oled_print(10, 24, "Plant Monitor");
    oled_print(20, 36, "Starting...");
    oled_display();
    delay(2000);
}
...
void loop() {
    ...
    // Send MQTT status update periodically
...
void runPump() {
    ...
    if (!pumpServiceEnabled) {
        return; // Skip pump logic if disabled
    }
